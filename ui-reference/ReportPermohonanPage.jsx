...
const DEFAULT_SIGNATORIES =
   DataService.getDefaultReportSignatories("UPT Semarang");

export const ReportPermohonanPage = ({ currentUser, submissions = [] }) => {
   const [activeSubTab, setActiveSubTab] = useState("pln_report"); // 'pln_report' | 'lembur_tidak_ditagihkan'
   const [selectedMonth, setSelectedMonth] = useState("all");
   const [selectedYear, setSelectedYear] = useState("2026");
   const [selectedType, setSelectedType] = useState("all");
   const [selectedLemburTagihan, setSelectedLemburTagihan] = useState("all");
   ...
   const [isPreviewReportModalOpen, setIsPreviewReportModalOpen] =
      useState(false);
   ...
   const userUnitScope = useMemo(() => {
      ...
      try {
         const masterPeg =
            MasterDataService.getAll("m_pegawai", { limit: 1000 })?.data || [];
         const pegRecord = masterPeg.find((p) => p.nip === currentUser.nip);
         if (pegRecord) {
            const masterUnit =
               MasterDataService.getAll("m_unit", { limit: 1000 })?.data || [];
            const unitRecord = masterUnit.find(
               (u) => u.id_unit_upt === pegRecord.id_unit_upt,
            );
   ...
   const availableUnitGroups = useMemo(() => {
      ...
      return allGroups.filter(
         (g) =>
            g.label.toLowerCase().includes(normalizedScope) ||
            normalizedScope.includes(g.label.toLowerCase()),
      );
   }, [userUnitScope]);
      ...
      try {
         ...
         if (saved) {
            ...
            if (Array.isArray(parsed)) {
               if (
                  parsed.length === 3 &&
                  (parsed[0].role?.includes("Verifikasi") ||
                     parsed[0].title === "DIVERIFIKASI OLEH")
               ) {
                  setSignatories([
                     {
                        ...parsed[1],
   ...
   const nonMakerSignatories = useMemo(() => {
      try {
         ...
         const finalUnitScope =
            userUnitScope || (selectedUnit !== "all" ? selectedUnit : null);
         ...
         const isMatchUpt = (unitName) => {
            ...
            if (
               nameLower.includes(scopeLower) ||
               scopeLower.includes(nameLower)
            )
               return true;

            if (scopeLower.includes("semarang")) {
               return (
                  nameLower.includes("semarang") ||
                  nameLower.includes("kudus") ||
                  nameLower.includes("srondol") ||
                  nameLower.includes("ungaran") ||
                  nameLower.includes("krapyak")
               );
            } else if (scopeLower.includes("purwokerto")) {
               return (
                  nameLower.includes("purwokerto") ||
                  nameLower.includes("kalisari")
               );
            } else if (scopeLower.includes("surakarta")) {
               return (
                  nameLower.includes("surakarta") || nameLower.includes("pedan")
               );
            } else if (scopeLower.includes("salatiga")) {
               return (
                  nameLower.includes("salatiga") ||
                  nameLower.includes("tuntang") ||
                  nameLower.includes("bawen")
               );
            }
         ...
         authUsers.forEach((u) => {
            ...
            const isAllowedRole =
               roleLower === "checker" ||
               roleLower === "verification" ||
               roleLower === "approved1" ||
               roleLower === "approval 1" ||
               roleLower === "approval1";
            const isActive =
               u.status === undefined ||
               u.status === "AKTIF" ||
            ...
            if (!userUptName && u.nip) {
               ...
               if (matchedPeg) {
                  const matchedUnit = masterUnits.find(
                     (un) => un.id_unit_upt === matchedPeg.id_unit_upt,
                  );
                  if (matchedUnit) {
            ...
            if (
               ...
               list.push({
                  ...
                  jabatan:
                     u.jabatan ||
                     (u.role ? u.role.toUpperCase() : "Pejabat PLN"),
         ...
         masterPegawai.forEach((p) => {
            ...
            if (!pegawaiUptName && p.id_unit_upt) {
               const matchedUnit = masterUnits.find(
                  (un) => un.id_unit_upt === p.id_unit_upt,
               );
               if (matchedUnit) {
   ...
   // Helper to check if submission is Lembur Tidak Ditagihkan (Piket Operator Cuti)
   const isNonBillableSub = (sub) => {
      if (sub.type !== "lembur") return false;
      const jp = sub.jenisPekerjaan;
      if (jp === "Pengganti Piket (Operator sedang cuti)") return true;
      if (typeof jp === "string") {
         const lower = jp.toLowerCase();
         return (
            lower.includes("pengganti piket") ||
            lower.includes("operator sedang cuti")
         );
      }
      return false;
   };
   ...
   // Separate dataset pools for Subtabs
   const plnApprovedSubmissions = useMemo(() => {
      return approved3Submissions.filter((sub) => !isNonBillableSub(sub));
   }, [approved3Submissions]);

   const nonBillableApprovedSubmissions = useMemo(() => {
      return approved3Submissions.filter((sub) => isNonBillableSub(sub));
   }, [approved3Submissions]);

   // Current Active Pool based on Subtab Selection
   const activePoolSubmissions = useMemo(() => {
      if (activeSubTab === "lembur_tidak_ditagihkan") {
         return nonBillableApprovedSubmissions;
      }
      return plnApprovedSubmissions;
   }, [activeSubTab, plnApprovedSubmissions, nonBillableApprovedSubmissions]);

   // Apply User Filters on Active Subtab Pool
   const filteredSubmissions = useMemo(() => {
      return activePoolSubmissions.filter((sub) => {
         // Month & Year Filter based on tanggalPengajuan / tanggalLembur / tanggalMulai
         const dateStr =
            sub.tanggalPengajuan || sub.tanggalLembur || sub.tanggalMulai;
         if (dateStr) {
         ...
         // Type Filter (only active in pln_report tab)
         if (activeSubTab === "pln_report") {
            if (selectedType === "lembur_tidak_ditagihkan") {
               if (!isNonBillableSub(sub)) return false;
            } else if (selectedType !== "all" && sub.type !== selectedType) {
               return false;
            }

            if (selectedLemburTagihan === "tidak_ditagihkan") {
               if (!isNonBillableSub(sub)) return false;
            } else if (selectedLemburTagihan === "ditagihkan") {
               if (sub.type === "lembur" && isNonBillableSub(sub)) return false;
            }
         }
         ...
         if (searchQuery.trim()) {
            ...
            const matchName = (sub.employeeName || "")
               .toLowerCase()
               .includes(q);
      ...
      activePoolSubmissions,
      activeSubTab,
      selectedMonth,
      selectedYear,
      selectedType,
      selectedLemburTagihan,
      selectedUnit,
      searchQuery,
   ]);
   ...
   const metrics = useMemo(() => {
      ...
      filteredSubmissions.forEach((s) => {
         const u = s.unitKerja || s.unitUltg || s.unitUpt || s.unit;
         if (u) unitsSet.add(u);
   ...
   const typeLabelMap = {
      ...
      sakit: "Sakit",
      sppd: "SPPD",
      lembur_tidak_ditagihkan: "Lembur Tidak Ditagihkan",
   };

   const filterInfo = {
      periode: `${selectedMonth === "all" ? "Semua Bulan" : monthNames.find((m) => m.value === selectedMonth)?.label} ${selectedYear}`,
      type:
         activeSubTab === "lembur_tidak_ditagihkan"
            ? "Lembur Tidak Ditagihkan"
            : typeLabelMap[selectedType] || selectedType,
      reportTitle:
         activeSubTab === "lembur_tidak_ditagihkan"
            ? "LAPORAN REKAPITULASI LEMBUR TIDAK DITAGIHKAN (PIKET OPERATOR CUTI) - SEMAR PLN ES"
            : "LAPORAN REKAPITULASI PERMOHONAN - SEMAR PLN ES",
      rawType: selectedType,
      subtab: activeSubTab,
      lemburTagihan: selectedLemburTagihan,
      unit: effectiveUnit,
   ...
   const handleOpenExportModal = (format) => {
      if (format === "excel") {
         try {
            setIsExporting(true);
            const currentFilterInfo = { ...filterInfo, docSeq };
            const filename =
               activeSubTab === "lembur_tidak_ditagihkan"
                  ? "Laporan_Resume_Lembur_Tidak_Ditagihkan_PLN.xlsx"
                  : "Laporan_Resume_PLN.xlsx";
            ExportService.exportReportToExcel(
               filteredSubmissions,
               currentFilterInfo,
               signatories,
               filename,
            ...
            setSuccessNotification(
               `Dokumen Laporan Excel (.xlsx) ${
                  activeSubTab === "lembur_tidak_ditagihkan"
                     ? "Lembur Tidak Ditagihkan"
                     : "Permohonan Selesai"
               } berhasil di-export (${filteredSubmissions.length} Data Transaksi).`,
            );
            setTimeout(() => setSuccessNotification(null), 4000);
         } catch (err) {
            console.error("Failed exporting Excel:", err);
         } finally {
   ...
   const handleSelectUserForSignatory = (index, userNip) => {
      ...
      setSignatories((prev) => {
         const next = [...prev];
         const defaultTitle =
            index === 0 ? "DIVERIFIKASI OLEH" : "MENGETAHUI / PENGESAHAN";
         const defaultRole =
            found.roleLabel === "CHECKER"
               ? "TL PLN (Diverifikasi / Approval 2)"
               : found.roleLabel === "APPROVED1"
                 ? "MAN PLN (Mengetahui / Approval 3)"
                 : found.roleLabel;
         next[index] = {
            ...next[index],
            name: found.name,
   ...
   const handleResetSignatories = () => {
      ...
      setSuccessNotification(
         "Pejabat Penandatangan di-reset ke data default (Diverifikasi & Mengetahui/Pengesahan).",
      );
   ...
   const handleSaveSignatoriesOnly = () => {
      try {
         localStorage.setItem(
            "epresensi_report_signatories",
            JSON.stringify(signatories),
         );
         setSuccessNotification(
            "Susunan Pejabat Penandatangan (Diverifikasi & Mengetahui) berhasil disimpan!",
         );
         setTimeout(() => setSuccessNotification(null), 4000);
      } catch (err) {
   ...
   const handleExecuteExport = async () => {
      ...
      try {
         ...
         setSuccessNotification(
            `Dokumen Laporan ${exportFormat.toUpperCase()} (#${String(currentSeq).padStart(3, "0")}) ${
               activeSubTab === "lembur_tidak_ditagihkan"
                  ? "Lembur Tidak Ditagihkan"
                  : "Permohonan Selesai"
            } berhasil di-generate dengan 2 Pejabat Penandatangan (Diverifikasi & Mengetahui/Pengesahan).`,
         );
   ...
   return (
      <div className="space-y-6 pb-20 select-none">
         ...
         <div className="bg-slate-200/80 p-1.5 rounded-2xl border border-slate-300/80 shadow-inner flex flex-col sm:flex-row items-stretch gap-1.5">
            <button
               type="button"
               onClick={() => {
                  setActiveSubTab("pln_report");
                  setSelectedType("all");
               }}
               className={`flex-1 px-4 py-3 rounded-xl font-black text-xs transition cursor-pointer flex items-center justify-center gap-2.5 ${
                  activeSubTab === "pln_report"
                     ? "bg-white text-slate-900 shadow-md border border-slate-200"
                     : "text-slate-600 hover:text-slate-900 hover:bg-white/50"
               }`}
            >
               <ShieldCheck
                  className={`w-4 h-4 ${activeSubTab === "pln_report" ? "text-emerald-600" : "text-slate-500"}`}
               />
               <span>Report Permohonan Selesai (Fokus Laporan PLN)</span>
               <span
            ...
            <button
               type="button"
               onClick={() => {
                  setActiveSubTab("lembur_tidak_ditagihkan");
                  setSelectedType("all");
               }}
               className={`flex-1 px-4 py-3 rounded-xl font-black text-xs transition cursor-pointer flex items-center justify-center gap-2.5 ${
                  activeSubTab === "lembur_tidak_ditagihkan"
            ...
            >
               <Clock
                  className={`w-4 h-4 ${activeSubTab === "lembur_tidak_ditagihkan" ? "text-amber-600" : "text-slate-500"}`}
               />
               <span>Lembur Tidak Ditagihkan (Piket Operator Cuti)</span>
               <span
                  className={`px-2 py-0.5 rounded-full text-[10px] font-extrabold ${
                     activeSubTab === "lembur_tidak_ditagihkan"
               ...
               >
                  {nonBillableApprovedSubmissions.length}
               </span>
            </button>
         </div>
         ...
         <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div>
               <div className="flex items-center gap-2.5">
                  <div
                     className={`p-2 rounded-xl ${activeSubTab === "lembur_tidak_ditagihkan" ? "bg-amber-100 text-amber-700" : "bg-emerald-100 text-emerald-700"}`}
                  >
                  ...
                  <div>
                     <h1 className="text-xl font-black text-slate-900 tracking-tight">
                        {activeSubTab === "lembur_tidak_ditagihkan"
                           ? "Report Lembur Tidak Ditagihkan (Piket Operator Cuti)"
                           : "Report Permohonan Selesai (Fokus Laporan PLN)"}
                     </h1>
                     <p className="text-xs text-slate-500 font-medium">
                        {activeSubTab === "lembur_tidak_ditagihkan"
                           ? "Rekapitulasi khusus permohonan lembur pengganti piket operator cuti (Non-Billable)"
                           : "Rekapitulasi resmi & laporan resume seluruh transaksi yang telah disetujui untuk laporan PLN"}
                     </p>
                  </div>
               </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 w-full md:w-auto shrink-0">
               ...
               <button
                  ...
                  title="Pratinjau tampilan dokumen laporan untuk periode saat ini"
               >
               ...
               <button
                  ...
                  className="flex-1 md:flex-none px-4 py-2.5 min-h-[42px] font-bold text-xs bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl shadow-md shadow-emerald-600/20 transition cursor-pointer flex items-center justify-center gap-2 disabled:opacity-50"
                  title="Export Rekapitulasi Laporan ke File Excel (.xlsx)"
               >
         ...
         <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs">
               ...
               <div className="flex items-baseline gap-1 mt-1">
                  <span
                     className={`text-lg font-black font-mono ${activeSubTab === "lembur_tidak_ditagihkan" ? "text-amber-600" : "text-sky-600"}`}
                  >
                     {activeSubTab === "lembur_tidak_ditagihkan"
                        ? "Non-Billable"
                        : formatRupiah(metrics.totalSppdCost)}
                  </span>
               </div>
               <p className="text-[10px] text-slate-400 font-medium mt-1">
                  {activeSubTab === "lembur_tidak_ditagihkan"
                     ? "Pengganti Operator Cuti"
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs">
               <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">
                  {activeSubTab === "lembur_tidak_ditagihkan"
                     ? "Cakupan Unit Terkait"
                     : "Total Cuti / Ijin / Sakit"}
               </p>
               {activeSubTab === "lembur_tidak_ditagihkan" ? (
                  <div>
                     <div className="flex items-baseline gap-1 mt-1">
                  ...
                  <div>
                     <div className="flex flex-wrap items-baseline gap-x-1.5 gap-y-0.5 mt-1">
                        <span className="text-sm font-black text-emerald-600">
                           {metrics.totalCutiDays} Cuti
                        </span>
                        <span className="text-[10px] font-bold text-slate-300">
                           /
                        </span>
                        <span className="text-sm font-black text-amber-600">
                           {metrics.totalIjinDays} Ijin
                        </span>
                        <span className="text-[10px] font-bold text-slate-300">
                           /
                        </span>
                        <span className="text-sm font-black text-rose-500">
                           {metrics.totalSakitDays} Sakit
                        </span>
                     </div>
                     <p className="text-[10px] text-slate-400 font-medium mt-1">
                        Rekapitulasi Cuti, Ijin & Sakit Pegawai
                     </p>
                  </div>
               )}
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 pb-3 border-b border-slate-100">
               <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-bold text-sm text-slate-800">
                     Filter Laporan & Parameter Pencarian
                  </h2>
               </div>
               {isPeriodCompleted ? (
                  <span className="text-[11px] font-bold text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-lg border border-emerald-200 inline-flex items-center gap-1">
                     ...
                     Periode Selesai (Siap Generate PDF)
                  </span>
               ) : (
                  <span className="text-[11px] font-bold text-amber-700 bg-amber-50 px-2.5 py-1 rounded-lg border border-amber-200 inline-flex items-center gap-1">
                     <Clock className="w-3.5 h-3.5 text-amber-600" /> Periode
                     Sedang Berlangsung (PDF Dibatasi)
                  </span>
               )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-3 text-xs">
               ...
               <div>
                  ...
                  >
                     ...
                     <option value="lembur_tidak_ditagihkan">
                        Lembur Tidak Ditagihkan
                     </option>
               ...
               {/* Separate Parameter Filter for Lembur Tidak Ditagihkan */}
               <div>
                  <label className="block font-bold text-slate-700 mb-1">
                     Parameter Tagihan Lembur
                  </label>
                  <select
                     value={selectedLemburTagihan}
                     onChange={(e) => setSelectedLemburTagihan(e.target.value)}
                     className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl font-bold text-slate-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/20 cursor-pointer"
                  >
                     <option value="all">Semua Status Lembur</option>
                     <option value="tidak_ditagihkan">
                        Lembur Tidak Ditagihkan (Piket Operator Cuti)
                     </option>
                     <option value="ditagihkan">
                        Lembur Ditagihkan (Regular)
                     </option>
                  </select>
               </div>
               ...
               <div>
                  ...
                  >
                     {userUnitScope ? (
                        <option value={userUnitScope}>
                           -- Semua Unit di {userUnitScope} --
                        </option>
                     ) : (
                        <option value="all">
                           -- Semua Unit (Seluruh UPT) --
                        </option>
                     )}
            ...
            <div className="px-5 py-4 border-b border-slate-200/80 flex items-center justify-between">
               <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-bold text-sm text-slate-900">
                     Daftar Permohonan Di Setujui ({filteredSubmissions.length})
                     berkas
                  </h3>
               </div>
                  ...
                  <tbody className="divide-y divide-slate-100 font-medium">
                     {filteredSubmissions.length === 0 ? (
                        <tr>
                           ...
                           >
                              <p className="font-bold text-slate-600 mb-1">
                                 Tidak ada dokumen yang telah di Setujui
                              </p>
                              <p>
                                 Tidak ditemukan data permohonan yang sesuai
                                 dengan filter pilihan Anda.
                              </p>
                           </td>
                        </tr>
                     ) : (
                        filteredSubmissions.map((sub, index) => {
                           ...
                           if (sub.type === "lembur")
                              rincian = `${sub.kategoriLembur || ""} - ${sub.jenisPekerjaan || ""} (${sub.durasiJam || 0} Jam)`;
                           if (sub.type === "cuti")
                              rincian = `${sub.cutiType || "Cuti"} (${sub.jumlahHari || 0} Hari)`;
                           if (sub.type === "sppd")
                              rincian = `${sub.maksudSppd || ""} - Rute: ${sub.kotaTujuan || "-"}`;
                           ...
                           return (
                              ...
                              >
                                 ...
                                 <td
                                    className="py-3.5 px-4 max-w-xs truncate text-slate-700"
                                    title={rincian}
                                 >
                                    {rincian}
                                 </td>
               ...
               >
                     ...
                     <div className="flex items-center gap-3">
                        ...
                        <div>
                           <h3 className="font-black text-base text-white tracking-tight flex items-center gap-2">
                              <span>
                                 Penentuan Pejabat Penandatangan Laporan
                              </span>
                           ...
                           <p className="text-xs text-slate-400">
                              Susun pejabat penandatangan yang akan dicetak
                              dalam dokumen laporan dari KIRI ke KANAN.
                           </p>
                        </div>
                     </div>
                  ...
                  <div className="p-6 space-y-6 max-h-[75vh] overflow-y-auto bg-slate-50/50">
                     ...
                     <div className="p-3.5 bg-amber-50 border border-amber-200 rounded-2xl text-xs text-amber-900 flex items-start gap-2.5">
                        ...
                        <p>
                           <strong>Panduan Tandatangan:</strong> Laporan akan
                           mencetak 2 blok penandatangan secara horisontal
                           (Posisi Kiri DIVERIFIKASI OLEH dan Posisi Kanan
                           MENGETAHUI / PENGESAHAN). Anda dapat memilih pejabat
                           dari sistem atau mengetikkan nama, NIP, dan jabatan
                           secara manual.
                        </p>
                     </div>
                        ...
                        {signatories.map((sig, idx) => (
                           ...
                           >
                              ...
                              <div>
                                 ...
                                 >
                                    <option value="">
                                       -- Pilih Pejabat / Pegawai Non-Maker --
                                    </option>
