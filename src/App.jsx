...
import { c } from "@/dist/assets/vendor-charts-DL1Agt5i";

function RouteAccessGuard({ moduleId, currentUser, children }) {
   const navigate = useNavigate();
   const hasAccess = ResponsibilityService.hasAccess(
      currentUser?.role,
      moduleId,
      currentUser?.nip,
   );
         ...
         <div className="p-6 max-w-lg mx-auto my-12 bg-white rounded-3xl shadow-xl border border-rose-100 text-center space-y-4">
            ...
            <div className="space-y-1">
               ...
               <p className="text-xs text-slate-600 font-medium leading-relaxed">
                  Peran Anda (<strong>{currentUser?.role || "Pengguna"}</strong>
                  ) tidak memiliki akses untuk membuka modul ini berdasarkan
                  Matriks Responsibility. Hubungi Administrator untuk
...
function AppContent() {
   ...
   const [isSidebarOpen, setIsSidebarOpen] = useState(() => {
      ...
      return (
         globalThis.appStorage.getItem("epresensi_sidebar_open") !== "false"
      );
   });
   ...
   const refreshData = () => {
      try {
         setFetchError(null);
         if (
            globalThis.appStorage.getItem("epresensi_simulate_fetch_error") ===
            "true"
         ) {
            throw new Error(
               "Network Timeout: Hubungan ke server pusat PT PLN Electricity Services terputus (504 Gateway Timeout).",
            );
         }
   ...
   useEffect(() => {
      ...
      Promise.all([
         ...
         .catch((error) => {
            if (!cancelled) {
               console.error(
                  "Gagal mengambil access module berdasarkan role:",
                  error,
               );
               toast.error(
                  error.response?.data?.message ||
                     error.message ||
                     "Gagal mengambil data master atau hak akses menu.",
               );
            }
         })
         .finally(() => {
   ...
   useEffect(() => {
      ...
      Promise.all([
         ...
         .then(
            ([unitResponse, projectResponse, jabatanResponse, unitRoles]) => {
               if (!active) return;
               setFilterMasterData({
         ...
         .catch((error) =>
            console.error("Gagal memuat scope filter Navbar:", error),
         );
      return () => {
         active = false;
      };
   ...
   const masterJabatans = useMemo(() => {
      try {
         return (
            MasterDataService.getAll("m_jabatan", { limit: 1000 })?.data || []
         );
      } catch {
         return [];
      }
   ...
   const masterProjects = useMemo(() => {
      ...
      try {
         return (
            MasterDataService.getAll("m_project", { limit: 1000 })?.data || []
         );
      } catch {
         return [];
      }
   ...
   const navbarScope = useMemo(() => {
      ...
      const unitById = new Map(
         masterUnits.map((unit) => [Number(unit.id_unit), unit]),
      );
      ...
      const ownUnitId = Number(
         person.id_unit || person.unit?.id_unit || currentUser?.id_unit || 0,
      );
      ...
      const getUnitKind = (unit) => {
         const name = String(unit?.nama_unit || "")
            .trim()
            .toUpperCase();
      ...
      const findPath = (kind) =>
         path.find((unit) => getUnitKind(unit) === kind);
      ...
      const activeAssignments = filterMasterData.unitRoles.filter(
         (assignment) =>
            assignment.is_active === "Y" && Number(assignment.id_unit),
      );
              ...
              new Map(
                 (activeAssignments.length
                    ? activeAssignments.flatMap((assignment) =>
                         assignment.scope_type === "SELF_AND_DESCENDANTS"
                            ? collectDescendants(assignment.id_unit)
                            : [unitById.get(Number(assignment.id_unit))].filter(
                                 Boolean,
                              ),
                      )
                    : ownChildren
      ...
      const selectableUnits = masterUnits.filter((unit) =>
         selectableIds.has(Number(unit.id_unit)),
      );
            ...
            : selectedUltg !== "Semua ULTG"
              ...
              : selectedUpt !== "Semua UPT"
                ...
                : selectedUp !== "Semua UP"
                  ? selectedUp
                  : "";
      const selectedUnit = masterUnits.find(
         (unit) => unit.nama_unit === selectedName,
      );
      ...
      const isPetugasAccount = Boolean(
      ...
      const petugasProjectId = Number(
         currentUser?.petugas?.id_project ||
            currentUser?.petugas?.project?.id_project ||
            person.id_project ||
            person.project?.id_project ||
            currentUser?.id_project ||
            0,
      );
      const assignedProjectIds = isPetugasAccount
         ...
         : pegawaiProjects
              .filter(
                 (project) =>
                    project.is_active !== "N" &&
                    project.PegawaiProject?.is_active !== "N",
              )
      ...
      const allowedProjectIds = isAdministrator
         ...
         : [...new Set(assignedProjectIds)].filter((id) =>
              masterProjects.some(
                 (project) => Number(project.id_project) === id,
              ),
           );
      const projectNames = masterProjects
         .filter((project) =>
            allowedProjectIds.includes(Number(project.id_project)),
         )
      ...
      return {
         ...
         allowedProjectIds,
         projectLabel:
            isAdministrator || allowedProjectIds.length > 1
               ? "Semua Project"
         ...
         ultgName: isAdministrator
            ? "Semua ULTG"
            : ultg?.nama_unit || "Semua ULTG",
         ...
         activeFilterUnitIds,
         selectableUnits,
         unitKindById: Object.fromEntries(
            masterUnits.map((unit) => [unit.id_unit, getUnitKind(unit)]),
         ),
      };
   }, [
      currentUser,
   ...
   const hierarchicalUnitLists = useMemo(() => {
      ...
      const kindOf = (unit) =>
         navbarScope.unitKindById?.[unit.id_unit] || "UNIT";
      ...
      const names = (kind, ...parents) =>
         units
            .filter(
               (unit) =>
                  kindOf(unit) === kind &&
                  parents.every((parent) => hasAncestor(unit, parent)),
            )
      ...
      masterUnits,
      navbarScope.selectableUnits,
      navbarScope.unitKindById,
      selectedUp,
      selectedUpt,
      selectedUltg,
   ]);
   ...
   const handleSwitchRole = (roleOrUser) => {
      ...
      if (updatedUser) {
         ...
         toast.success(
            `Berhasil beralih peran ke: ${targetRole.toUpperCase()}`,
         );
      } else {
   ...
   const getSubmissionPrimaryDate = (sub) => {
      if (sub.type === "lembur")
         return sub.tanggalLembur || sub.tanggalPengajuan || "";
      if (sub.type === "cuti")
         return sub.tanggalMulai || sub.tanggalPengajuan || "";
      if (sub.type === "ijin")
         return sub.tanggalMulai || sub.tanggalPengajuan || "";
      if (sub.type === "sakit")
         return sub.tanggalMulai || sub.tanggalPengajuan || "";
      if (sub.type === "sppd")
         return sub.tanggalBerangkat || sub.tanggalPengajuan || "";
   ...
   const getProjectIdForSubmission = useCallback((sub, usersList, jabatans) => {
      ...
      if (employee) {
         const matchedJab = (jabatans || []).find(
            (j) =>
               j.nama_jabatan?.toLowerCase() ===
               (employee.jabatan || "").toLowerCase(),
         );
         if (matchedJab) {
      ...
      const matchedJab = (jabatans || []).find(
         (j) =>
            j.nama_jabatan?.toLowerCase() ===
            (sub.employeeJabatan || "").toLowerCase(),
      );
      if (matchedJab) {
         return String(matchedJab.id_project);
      }
   ...
   const getProjectNameForSubmission = useCallback(
      (sub, usersList, jabatans, projects) => {
         const employee = (usersList || []).find(
            (u) => u.nip === sub.employeeNip,
         );
         if (employee) {
            if (
               employee.multiProject &&
               Array.isArray(employee.multiProject) &&
               employee.multiProject.length > 0
            ) {
               const firstProjId = employee.multiProject[0];
               const proj = (projects || []).find(
                  (p) => String(p.id_project) === String(firstProjId),
               );
            ...
            if (matchedJab) {
               const proj = (projects || []).find(
                  (p) => Number(p.id_project) === Number(matchedJab.id_project),
               );
               if (proj) return proj.nama_project;
            }
         }
         ...
         return (
            (projects && projects[0]?.nama_project) || "Operator Gardu Induk"
         );
      },
      [],
   );
   ...
   const filteredSubmissions = useMemo(() => {
      return submissions.filter((sub) => {
         ...
         if (
            !navbarScope.isAdministrator &&
            navbarScope.allowedProjectIds.length
         ) {
            const subProjectId = Number(
               getProjectIdForSubmission(sub, allUsers, masterJabatans),
            );
            if (!navbarScope.allowedProjectIds.includes(subProjectId))
               return false;
         }

         // 3. Global unit filters
         const emp = (allUsers || []).find((u) => u.nip === sub.employeeNip);
         const submissionUnitId = Number(
            sub.id_unit || emp?.id_unit || emp?.petugas?.id_unit || 0,
         );
         const hasSpecificUnitFilter = [
            selectedUp,
            selectedUpt,
            selectedUltg,
            selectedGi,
         ...
         if (
            (!navbarScope.isAdministrator || hasSpecificUnitFilter) &&
            navbarScope.activeFilterUnitIds?.length
         ) {
            if (
               !submissionUnitId ||
               !navbarScope.activeFilterUnitIds.includes(submissionUnitId)
            )
               return false;
         }
   ...
   const activeNavbarProjectIds = useMemo(() => {
      const selected = masterProjects.find(
         (project) => project.nama_project === selectedProject,
      );
      ...
      selectedProject,
      navbarScope.isAdministrator,
      navbarScope.allowedProjectIds,
   ]);

   const navbarProjectList = useMemo(
      () =>
         navbarScope.isAdministrator
            ? masterProjects
            : masterProjects.filter((project) =>
                 navbarScope.allowedProjectIds.includes(
                    Number(project.id_project),
                 ),
              ),
      [
         masterProjects,
   ...
   const selectNavbarGi = useCallback(
      (unitName) => {
         if (!unitName) {
            setSelectedUpt(navbarScope.uptName);
            setSelectedUltg(navbarScope.ultgName);
         ...
         setSelectedUpt(
            path.find((unit) => /^UPT\b/i.test(unit.nama_unit))?.nama_unit ||
               navbarScope.uptName,
         );
         setSelectedUltg(
            path.find((unit) => /^ULTG\b/i.test(unit.nama_unit))?.nama_unit ||
               navbarScope.ultgName,
         );
   ...
   const handleTabChange = (tabId) => {
      const isMaker =
         currentUser?.role === "maker" || currentUser?.role === "superadmin";
      if (
         !isMaker &&
         ["lembur", "cuti", "ijin", "sakit", "sppd"].includes(tabId)
      ) {
         const routeMap = {
         ...
         navigate(routeMap[currentUser?.role] || "/workflow", {
            state: { typeFilter: tabId },
         });
         setActiveTab(tabId);
         return;
      }
   ...
   const notifications = currentUser
      ? DataService.getNotifications(currentUser.nip)
      : [];
            ...
            <div className="bg-white rounded-3xl shadow-xl border border-slate-200/60 max-w-lg w-full p-6 space-y-5 text-center">
               ...
               <div className="space-y-2">
                  <h2 className="text-lg font-black text-slate-900">
                     Gagal Memuat Profil & Pengaturan
                  </h2>
                  <p className="text-xs text-slate-500 font-semibold leading-relaxed">
                     Sistem mendeteksi adanya gangguan koneksi ke server PLN
                     digital core atau data profile bermasalah.
                  </p>
               </div>
               ...
               <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-3">
                  ...
                  {globalThis.appStorage.getItem(
                     "epresensi_simulate_fetch_error",
                  ) === "true" && (
                     <button
                        onClick={() => {
                           globalThis.appStorage.removeItem(
                              "epresensi_simulate_fetch_error",
                           );
                           refreshData();
                        }}
      ...
      <Routes>
            ...
            element={
               isAccessLoading ? (
                  ...
                  <MainLayout
                     ...
                     projectReadOnly={
                        !navbarScope.isAdministrator &&
                        navbarScope.allowedProjectIds.length <= 1
                     }
         ...
         >
            ...
            <Route
               path="lembur"
               element={
                  !["maker", "superadmin"].includes(currentUser?.role) ? (
                     <Navigate
                        to="/workflow"
                        state={{ typeFilter: "lembur" }}
                        replace
                     />
                  ) : (
                     <LemburPage
            ...
            />
            <Route
               path="perintah-kerja-lembur"
               element={
                  <RouteAccessGuard
                     moduleId="perintah-kerja-lembur"
                     currentUser={currentUser}
                  >
                     <PerintahKerjaLemburPage
                        currentUser={currentUser}
                        navbarScope={navbarScope}
                        startDate={startDate}
                        endDate={endDate}
            ...
            <Route
               path="workflow/maker"
               element={<WorkflowApprovalPage stage="MAKER" />}
            />
            <Route
               path="workflow/checker"
               element={<WorkflowApprovalPage stage="CHECKER" />}
            />
            <Route
               path="workflow/verification"
               element={<WorkflowApprovalPage stage="VERIFICATION" />}
            />
            <Route
               path="workflow/approval-1"
               element={<WorkflowApprovalPage stage="APPROVAL_1" />}
            />
            <Route
               path="workflow/approval-2"
               element={<WorkflowApprovalPage stage="APPROVAL_2" />}
            />
            <Route
               path="workflow/approval-3"
               element={<WorkflowApprovalPage stage="APPROVAL_3" />}
            />
            ...
            <Route
               path="pengaturan/access-matrix"
               element={<Navigate to="/responsibilities" replace />}
            />
         </Route>
