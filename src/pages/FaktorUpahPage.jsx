...
import { UpahPegawaiTab } from "./UpahPegawaiTab";
...
const FaktorKoefTab = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    const masaKerja = Number(form.masa_kerja);
    if (!Number.isInteger(masaKerja) || masaKerja < 0) {
      setFormError("Masa kerja minimum harus berupa tahun penuh dan minimal 0."); return;
    }
    const payload = { masa_kerja: masaKerja, koef, tmk, keterangan: form.keterangan.trim(), is_active: form.is_active };
  ...
  return <div className="space-y-5">
    ...
    {isFormOpen && <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4"><div className="bg-white w-full max-w-lg rounded-t-3xl sm:rounded-3xl shadow-2xl"><div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between"><div><h2 className="text-base font-black">{editingRecord ? "Edit" : "Tambah"} KOEF &amp; TMK</h2><p className="text-xs text-slate-500">Form sesuai body API koef-tmk.</p></div><button onClick={() => setIsFormOpen(false)} className="w-9 h-9 flex items-center justify-center rounded-xl hover:bg-slate-100"><X className="w-5 h-5" /></button></div><form onSubmit={handleSubmit} className="p-5 space-y-4">
      <label className="space-y-1.5 block"><span className="text-xs font-bold text-slate-700">Minimum Masa Kerja (tahun) *</span><input type="number" min="0" step="1" value={form.masa_kerja} onChange={(e) => updateForm("masa_kerja", e.target.value)} placeholder="Contoh: 3" className="form-input" required /><span className="text-[10px] text-slate-500">Tier berlaku mulai masa kerja ini sampai sebelum tier berikutnya.</span></label>
