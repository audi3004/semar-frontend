  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (editingRecord) await api.client.put(`${endpoint}/${recordId}`, payload);
        ...
        const saved = await api.client.post(endpoint, payload);
        savedId = saved.data?.data?.id_pegawai || saved.data?.data?.id_petugas;
      }
      if (activeTab === "pegawai" && savedId) {
        await api.client.put(`/pegawai/${savedId}/projects`, { project_ids: form.project_ids });
      }
          ...
          <tbody className="divide-y divide-slate-100">
            {pagination.paginatedItems.map((item) => (
              <tr key={activeTab === "pegawai" ? item.id_pegawai : item.id_petugas} className="hover:bg-slate-50">
                ...
                {activeTab === "petugas" && <td className="px-4 py-3 text-right">{item.perhitungan_upah?.status_perhitungan === "OK" ? <><div className="font-black text-emerald-700">{rupiah.format(Number(item.perhitungan_upah.total_gaji || 0))}</div><div className="mt-0.5 text-[10px] text-slate-500">Tarif lembur {rupiah.format(Number(item.perhitungan_upah.tarif_lembur ?? item.perhitungan_upah.tarif_lembur_per_jam ?? 0))}</div></> : <span className="inline-block max-w-44 text-[10px] font-bold leading-relaxed text-rose-600">{item.perhitungan_upah?.status_perhitungan || "Perhitungan belum tersedia"}</span>}</td>}
