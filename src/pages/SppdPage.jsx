...
export const SppdPage = ({ currentUser, submissions, onRefreshData }) => {
   ...
   const [isApprover2SppdModalOpen, setIsApprover2SppdModalOpen] =
      useState(false);
   ...
   const normalizeStatus = (item) => {
      ...
      if (
         item?.status?.is_final === "Y" &&
         (code.includes("reject") || code.includes("tolak"))
      )
         return "rejected";
      if (item?.status?.is_final === "Y") return "approved";
      if (code.includes("revision") || code.includes("revisi"))
         return "revision";
      if (item?.status?.is_initial === "Y" || code.includes("draft"))
         return "draft";
      ...
      const expensesData = [
         {
            id: "akomodasi",
            kategori: "Akomodasi",
            deskripsi: item.desc_akomodasi || "Biaya Akomodasi",
            nominal: Number(item.rp_akomodasi || 0),
         },
         {
            id: "transportasi",
            kategori: "Transportasi",
            deskripsi: item.desc_transportasi || "Biaya Transportasi",
            nominal: Number(item.rp_transportasi || 0),
         },
         {
            id: "lain-lain",
            kategori: "Lain-lain",
            deskripsi: item.desc_lain_lain || "Biaya Lain-lain",
            nominal: Number(item.rp_lain_lain || 0),
         },
      ].filter(
         (entry) => entry.nominal > 0 || !entry.deskripsi.startsWith("Biaya "),
      );
      return {
         ...item,
         ...
         tanggalPengajuan:
            item.created_at?.slice?.(0, 10) || item.tgl_berangkat,
         ...
         totalEstimasiBiaya: expensesData.reduce(
      ...
      if (tanggalBerangkat && tanggalKembali) {
         ...
         if (!isNaN(start.getTime()) && !isNaN(end.getTime())) {
            ...
            const diffDays = Math.max(
               1,
               Math.ceil(diffTime / (1000 * 3600 * 24)) + 1,
            );
   ...
   const expenseByCategory = (category) =>
      expenses.filter((item) => item.kategori === category);
   const expenseTotal = (category) =>
      expenseByCategory(category).reduce(
         (sum, item) => sum + Number(item.nominal || 0),
         0,
      );
   const expenseDescription = (category) =>
   ...
   const buildApiPayload = () => {
      ...
      if (!idPetugas)
         throw new Error(
            "Akun login belum terhubung dengan data petugas (id_petugas).",
         );
      const values = {
   ...
   const handleOpenApproveFlow = (sub) => {
      ...
      if (effectiveRole === "checker") {
         setCheckerExpenses(
            sub.expenses && sub.expenses.length > 0
               ...
               : [
                    {
                       id: "exp-1",
                       deskripsi: "Biaya Transportasi Perjalanan Dinas",
                       kategori: "Transportasi",
                       nominal: 0,
                    },
                 ],
         ...
         effectiveRole === "approved1" ||
         effectiveRole === "approved2"
      ) {
   ...
   const handleResubmit = async (sub) => {
      try {
         ...
         setAlertModal({
            isOpen: true,
            type: "error",
            title: "Gagal Mengirim SPPD",
            message: getApiError(error),
         });
      }
   };
   const totalEstimasiBiaya = expenses.reduce(
      (a, b) => a + (b.nominal || 0),
      0,
   );
   ...
   const handleCreateSppd = async (e) => {
      ...
      if (
         ...
         setAlertModal({
            ...
            message:
               "Semua kolom formulir (Nomor Surat Tugas, Maksud Perjalanan, Kota Asal, Kota Tujuan, Tanggal Berangkat, Tanggal Kembali, dan Beban Anggaran) harus terisi dan lengkap sebelum dikirim.",
         });
         return;
      }
      ...
      const valResult = validateSppdInput(
         tanggalBerangkat,
         tanggalKembali,
         todayStr,
      );
      ...
      try {
         ...
         if (!id)
            throw new Error("ID SPPD tidak ditemukan pada respons server.");
         ...
         setAlertModal({
            ...
            message: "Pengajuan SPPD berhasil dikirim ke tahap berikutnya.",
         });
         await refreshSppd();
      } catch (error) {
         setAlertModal({
   ...
   const handleSubmitDraftDirectly = async (sub) => {
      if (
         !sub.nomorSuratTugas ||
         !sub.maksudPerjalanan ||
         !sub.kotaAsal ||
         !sub.kotaTujuan ||
         !sub.tanggalBerangkat ||
         !sub.tanggalKembali ||
         ...
         setAlertModal({
            ...
            message:
               "Pengajuan SPPD ini belum lengkap. Harap lengkapi semua kolom formulir dan tandatangan terlebih dahulu sebelum mengirim.",
         });
         return;
      }
      const todayStr = new Date().toISOString().slice(0, 10);
      ...
      try {
         ...
         setAlertModal({
            ...
            type: "success",
            title: "Pengajuan Berhasil Dikirim",
            message: `Draft pengajuan SPPD (${sub.nomorDokumen}) berhasil dikirim ke tahap berikutnya.`,
         });
         await refreshSppd();
      } catch (error) {
         setAlertModal({
         ...
         <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
            <div>
               <h1 className="text-lg sm:text-xl font-black text-slate-900 tracking-tight flex items-center gap-2">
                  <Briefcase className="w-5 h-5 sm:w-6 sm:h-6 text-sky-600" />{" "}
                  SPPD (Surat Perintah Perjalanan Dinas)
               </h1>
               <p className="text-xs text-slate-600 font-medium">
                  Pengajuan Biaya Perjalanan Dinas Berdasar Surat Tugas Resmi
                  PLN
               </p>
            </div>
            ...
            <div className="p-3.5 sm:p-4 border-b border-slate-200 bg-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-3">
               ...
               <div className="flex flex-wrap items-center gap-1.5">
                  ...
                  >
                     ...
                     Perlu Perbaikan (
                     {
                        sppdSubmissions.filter((s) => {
                           const sL = s.status ? s.status.toLowerCase() : "";
                           return (
                              sL === "revision" ||
                              sL === "revision_required" ||
                              sL === "rejected"
                           );
                        }).length
                     }
            ...
            <div className="block sm:hidden divide-y divide-slate-100">
               {displaySubmissions.length === 0 ? (
                  <div className="p-6 text-center text-slate-400 text-xs font-medium">
                     Belum ada pengajuan SPPD yang sesuai filter.
                  </div>
               ) : (
                  displaySubmissions.map((sub) => {
                     ...
                     return (
                        <div
                           key={sub.id}
                           className={`p-3.5 space-y-2.5 hover:bg-slate-50 transition ${isRevision ? "bg-amber-50/80 border-l-4 border-l-amber-500" : "bg-white"}`}
                        >
                           ...
                           <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-200/60 text-xs space-y-1">
                              <p className="font-semibold text-sky-800">
                                 {sub.kotaAsal} &rarr; {sub.kotaTujuan}
                              </p>
                              <div className="flex items-center justify-between text-[11px] text-slate-600">
                                 <span>
                                    {formatDateIndonesian(sub.tanggalBerangkat)}
                                 </span>
                              ...
                              <div className="flex items-center justify-between pt-1 border-t border-slate-200/40 text-[11px] font-bold">
                                 ...
                                 <span
                                    className={`font-mono ${sub.totalEstimasiBiaya > 0 || sub.expenses?.some((e) => Number(e.nominal) > 0) ? "text-emerald-700" : "text-slate-500"}`}
                                 >
                                    {formatRupiah(
                                       sub.totalEstimasiBiaya &&
                                          Number(sub.totalEstimasiBiaya) > 0
                                          ? sub.totalEstimasiBiaya
                                          : (sub.expenses || []).reduce(
                                               (acc, e) =>
                                                  acc +
                                 ...
                                 loggedInNip === sub.employeeNip && (
                                    <>
                                       ...
                                       >
                                          ...
                                          <ActionTooltip text="Kirim Ulang" />
                                       </button>
                                    </>
                                 )}
                                    ...
                                    <>
                                       ...
                                       <button
                                          onClick={() => {
                                             if (
                                                window.confirm(
                                                   "Apakah Anda yakin ingin membatalkan draft SPPD ini? Status akan diubah menjadi Dibatalkan.",
                                                )
                                             ) {
                     ...
                     {displaySubmissions.map((sub) => {
                        ...
                        return (
                           <React.Fragment key={sub.id}>
                              <tr
                                 className={`hover:bg-slate-50 transition ${isRevision ? "bg-amber-50/50" : ""}`}
                              >
                                 ...
                                 <td
                                    className={`p-3 font-mono font-bold ${sub.totalEstimasiBiaya > 0 || sub.expenses?.some((e) => Number(e.nominal) > 0) ? "text-emerald-700" : "text-slate-500"}`}
                                 >
                                    {formatRupiah(
                                       sub.totalEstimasiBiaya &&
                                          Number(sub.totalEstimasiBiaya) > 0
            ...
            <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-slate-900/60 backdrop-blur-xs p-0 sm:p-4 md:p-6">
               <div className="bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl border border-slate-200 max-w-lg md:max-w-2xl w-full p-4 sm:p-6 md:p-7 space-y-4 my-0 sm:my-8 max-h-[92vh] overflow-y-auto overscroll-y-contain">
                  <h3 className="text-base font-bold text-slate-900 flex items-center gap-2 border-b border-slate-100 pb-3">
                     ...
                     {editingSub
                        ? "Edit & Perbarui SPPD Perjalanan Dinas"
                        : "Formulir Pengajuan SPPD Perjalanan Dinas"}
                  </h3>

                  <form
                     onSubmit={handleCreateSppd}
                     className="space-y-3.5 text-xs"
                  >
                     <div>
                        <label className="block font-bold text-slate-800 mb-1">
                           Nomor Dasar Surat Tugas PLN
                        </label>
                        <input
                           type="text"
                     ...
                     <div className="p-3.5 bg-sky-50/80 border border-sky-200/80 rounded-2xl text-xs space-y-1">
                        ...
                        <p className="text-slate-600 text-[11px] leading-relaxed font-medium">
                           Rincian komponen biaya SPPD akan ditentukan oleh{" "}
                           <strong>TL PLN (Checker)</strong> saat review, dan
                           nominal biaya rupiah akan diisi oleh{" "}
                           <strong>TL ES (Approved 2)</strong>.
                        </p>
                     </div>

                     <div>
                        <label className="block font-bold text-slate-800 mb-1">
                           Tandatangan Canvas Digital Pemohon
                        </label>
                        {makerSignatureUrl ? (
                           <div className="p-3 border border-emerald-300 rounded-xl flex items-center justify-between bg-emerald-50">
                              ...
                              <button
                                 ...
                                 className="text-sky-700 font-bold underline cursor-pointer"
                              >
                                 Ubah
                              </button>
                           </div>
                  ...
                  <div className="space-y-3 text-xs">
                     <div className="bg-sky-50/80 p-3 rounded-2xl border border-sky-200 space-y-1">
                        <p className="font-bold text-slate-900">
                           {approveSub.employeeName} ({approveSub.employeeNip})
                        </p>
                        <p className="text-slate-700 font-semibold">
                           {approveSub.maksudPerjalanan}
                        </p>
                        <p className="text-slate-600">
                           Rute: {approveSub.kotaAsal} &rarr;{" "}
                           {approveSub.kotaTujuan} ({approveSub.durasiHari}{" "}
                           Hari)
                        </p>
                        <p className="text-sky-800 text-[11px] font-semibold pt-1 border-t border-sky-200/60 mt-1">
                           * Pilih kategori komponen biaya (Transportasi,
                           Akomodasi, Lain-lain). Nominal bersifat Readonly (Rp
                           0) &amp; akan diisi oleh Approved 2.
                        </p>
                     </div>
                        ...
                        onClick={() => {
                           if (checkerExpenses.length === 0) {
                              setAlertModal({
                                 ...
                                 message:
                                    "Harap tambahkan minimal 1 komponen biaya SPPD.",
                              });
                              return;
                           }
                           const hasEmptyDesc = checkerExpenses.some(
                              (e) => !e.deskripsi || !e.deskripsi.trim(),
                           );
                           if (hasEmptyDesc) {
                              setAlertModal({
                                 isOpen: true,
                                 ...
                                 message:
                                    "Harap isi deskripsi untuk seluruh komponen biaya SPPD.",
                              });
                              return;
                           }
                           setIsCheckerSppdModalOpen(false);
               ...
               <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 max-w-lg w-full p-6 space-y-4 max-h-[90vh] overflow-y-auto">
                  ...
                  <div className="space-y-3 text-xs">
                     <div className="bg-emerald-50/80 p-3 rounded-2xl border border-emerald-200 space-y-1">
                        ...
                        <p className="text-emerald-800 text-[11px] font-bold pt-1 border-t border-emerald-200/60 mt-1">
                           * Wajib memasukkan nominal rupiah untuk setiap
                           komponen biaya SPPD yang telah ditentukan Checker.
                        </p>
                     </div>
                  ...
                  <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
                     ...
                     <button
                        type="button"
                        onClick={() => {
                           const hasZero = approver2Expenses.some(
                              (e) => !e.nominal || Number(e.nominal) <= 0,
                           );
                           if (hasZero) {
                              setAlertModal({
                                 isOpen: true,
                                 ...
                                 message:
                                    "Harap isi nominal rupiah yang valid (lebih besar dari Rp 0) untuk semua komponen biaya SPPD.",
                              });
                              return;
                           }
                           setIsApprover2SppdModalOpen(false);
                     ...
                     >
                        <Check className="w-4 h-4" /> Simpan Nominal &amp;
                        Lanjut Tandatangan
                     </button>
                  </div>
               </div>
