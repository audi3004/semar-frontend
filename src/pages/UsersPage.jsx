...
import { Edit2, KeyRound, Plus, Search, UserCog, X } from "lucide-react";
...
export const UsersPage = ({ currentUser }) => {
  ...
  const canResetPassword = ["admin", "superadmin"].includes(currentUser?.role);
  ...
  const openResetPassword = (user) => {
    const initial = birthDatePassword(user.petugas?.tgl_lahir);
    setResetUser(user);
    setResetPassword(initial);
    setResetConfirm(initial);
    setResetError("");
  };
  ...
  const submitResetPassword = async (event) => {
    ...
    setResetError("");
    try {
      await api.client.patch(`/users/${resetUser.id_user}/reset-password`, {
      ...
      setResetError(error.response?.data?.message || "Gagal mereset password user.");
    } finally {
  ...
  return <div className="p-3 sm:p-6 space-y-5">
    ...
    {resetUser && <div className="fixed inset-0 z-[60] bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4"><form onSubmit={submitResetPassword} className="w-full max-w-md rounded-3xl bg-white shadow-2xl overflow-hidden"><div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between"><div><h2 className="font-black text-slate-900">Reset Password User</h2><p className="text-xs text-slate-500 mt-0.5">{resetUser.username} — {(resetUser.pegawai || resetUser.petugas)?.nama || "Tanpa identitas"}</p></div><button type="button" onClick={() => setResetUser(null)} className="p-2 rounded-xl hover:bg-slate-100"><X className="w-5 h-5" /></button></div><div className="p-5 space-y-4"><div className="rounded-xl bg-amber-50 border border-amber-200 p-3 text-[11px] text-amber-800">Untuk akun Petugas, password default menggunakan tanggal lahir dengan format <strong>DDMMYYYY</strong>.</div><Field label="Password Baru" required><input type="text" minLength={8} maxLength={100} value={resetPassword} onChange={(e) => setResetPassword(e.target.value)} className="form-input" required /></Field><Field label="Konfirmasi Password" required><input type="text" minLength={8} maxLength={100} value={resetConfirm} onChange={(e) => setResetConfirm(e.target.value)} className="form-input" required /></Field>{!resetUser.petugas?.tgl_lahir && <p className="text-[11px] font-bold text-rose-600">Tanggal lahir Petugas tidak tersedia. Masukkan password reset secara manual.</p>}{resetError && <div className="p-3 rounded-xl bg-rose-50 border border-rose-200 text-xs font-bold text-rose-700">{resetError}</div>}</div><div className="px-5 py-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-2"><button type="button" onClick={() => setResetUser(null)} className="px-4 py-2 rounded-xl bg-white border border-slate-200 text-xs font-bold">Batal</button><button disabled={isResetting || resetPassword.length < 8} className="px-4 py-2 rounded-xl bg-amber-600 text-white text-xs font-black disabled:opacity-50">{isResetting ? "Mereset..." : "Reset Password"}</button></div></form></div>}
  </div>;
};
