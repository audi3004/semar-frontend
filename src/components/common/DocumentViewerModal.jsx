...
export const DocumentViewerModal = ({
  ...
  }, [isOpen, viewTab, submission, isReport, reportSubmissions, reportFilterInfo, reportSignatories]);
  ...
  const handleDownloadPDF = () => {
    if (isReport) {
      PdfService.downloadReportPdf(reportSubmissions, reportFilterInfo, reportSignatories);
    } else if (submission) {
  ...
  const statusLower = submission ? (submission.status || "").toLowerCase() : "";
  const currentRoleLower = submission ? (submission.currentApproverRole || "").toLowerCase() : "";
  ...
  const renderSignatureBox = (titleOrStep, stepParam, isMaker = false) => {
    ...
    const isApprovedStep = isMaker ? true : (stepStatus === "approved" || stepStatus === "DISETUJUI" || Boolean(step?.signatureUrl) || Boolean(roleSigKey && submission[roleSigKey]));
    const isRejectedStep = !isMaker && (stepStatus === "rejected" || stepStatus === "DITOLAK");
    ...
    return (
      <div className="border border-black p-2 flex flex-col justify-between h-36 bg-white rounded-none">
        <div>
          <div className={`flex items-center ${isMaker ? "justify-between" : "justify-end"} border-b border-black pb-1 mb-1`}>
            {isMaker && (
          ...
          <p className="font-bold text-black text-[10.5px] truncate leading-tight">{name}</p>
          <p className="text-[9.5px] text-black/80 truncate leading-tight">{nip ? `NIP. ${nip}` : roleText}</p>
        </div>

        {/* Natural Signature Slot */}
        <div className="my-0.5 h-12 flex items-center justify-center bg-transparent relative">
          {sigUrl ? (
            <img
              src={sigUrl}
              alt={`TTD ${title}`}
              className="max-h-12 max-w-full object-contain mix-blend-multiply select-none"
            />
          ) : isSkipped ? (
            <span className="text-black font-semibold text-[8.5px] italic">[ DILEWATI ]</span>
          ) : isApprovedStep ? (
            <div className="text-center">
              <span className="text-black font-black text-[9.5px] tracking-wider block">[ TTD VALID ]</span>
              <span className="text-[7.5px] text-black font-mono">Digital Verified</span>
            </div>
          ) : isRejectedStep ? (
            <span className="text-black font-bold text-[9px] italic">[ DITOLAK ]</span>
          ) : (
        ...
        <div className="text-[8.5px] pt-1 border-t border-black/80 text-black">
          ...
          {!isMaker && (step?.notes || step?.catatan) && (
            <p className="text-[8px] italic text-black truncate mt-0.5" title={step.notes || step.catatan}>
  ...
  const isFullyApproved = submission && (submission.status === "APPROVED" || submission.status === "approved");
  const hasAttachment = submission && (
    (submission.type === "lembur" && !!(submission.fotoDokumentasi1Url || submission.fotoDokumentasi2Url || submission.dasarPerintahLemburUrl)) ||
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-2 sm:p-4 overflow-y-auto select-none modal-backdrop">
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xs sm:text-sm font-black tracking-tight text-white uppercase flex items-center gap-2">
                <span>{isReport ? "PRATINJAU DOKUMEN LAPORAN RESUME" : "DOKUMEN FORMAL DIGITAL"}</span>
                {isReport && (
                  <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full border uppercase ${
                    ...
                    {!isPeriodCompleted ? "Periode Saat Ini (Draft)" : "Periode Selesai"}
                  </span>
                )}
              </h3>
              <p className="text-[10px] sm:text-xs text-slate-400 font-mono">
                {isReport ? `PERIODE: ${reportFilterInfo?.periode || "Semua"} | APPROVED 3` : `ID DOKUMEN: ${finalDocNo}`}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between sm:justify-end gap-2">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button
                ...
                disabled={isReport && !isPeriodCompleted}
        ...
        {/* Modal Main Body */}
        <div className="flex-1 overflow-y-auto bg-slate-200/90 print:bg-white text-black p-2 sm:p-6 flex flex-col">
          {viewTab === "pdf" ? (
            ...
            <div className="flex-1 min-h-[500px] flex flex-col gap-3 no-print">
              {isReport && !isPeriodCompleted && (
                <div className="p-3 bg-amber-50 border border-amber-200 rounded-2xl text-xs text-amber-900 flex items-center gap-2 shrink-0">
                  ...
                  <span>
                    <strong>Mode Pratinjau Periode Saat Ini:</strong> Ini adalah pratinjau real-time untuk periode yang sedang berlangsung. Dokumen PDF resmi siap diunduh setelah periode berakhir.
                  </span>
                </div>
              )}
              ...
              {isGeneratingPdf ? (
                <div className="flex-1 min-h-[400px] flex flex-col items-center justify-center gap-3 text-slate-500 bg-white rounded-2xl border border-slate-300 shadow-xs">
                  ...
                  <p className="text-xs font-bold text-slate-700">Mebuat Pratinjau PDF Resmi...</p>
                  <p className="text-[11px] text-slate-400">Harap tunggu sebentar</p>
                </div>
              ) : pdfPreviewUrl ? (
                  ...
                  >
                    ...
                    <div className="p-8 bg-slate-50 rounded-2xl text-center flex flex-col items-center justify-center gap-4 h-full min-h-[450px]">
                      ...
                      <div className="max-w-md space-y-1">
                        <h4 className="font-bold text-slate-800 text-sm">Dokumen PDF Berhasil Dibuat</h4>
                        <p className="text-xs text-slate-500 leading-relaxed">
                          Keamanan browser Chrome membatasi tampilan PDF interaktif di dalam frame sandbox. Anda dapat membuka atau mengunduh PDF secara langsung di bawah ini.
                        </p>
                      </div>
            ...
            <div className="printable-a4-document max-w-[210mm] w-full mx-auto bg-white border border-black p-5 sm:p-6 space-y-4 rounded-none text-black">
              {/* Kop Surat Header */}
              <div className="border-b-2 border-black pb-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 page-break-inside-avoid">
                ...
                <div className="text-left sm:text-right text-xs text-black border-l sm:border-l-0 pl-3 sm:pl-0 border-black">
                  <p className="font-extrabold text-black uppercase tracking-tight">LAPORAN RESUME APPROVED 3</p>
                  <p className="font-mono text-black font-bold">Periode: {reportFilterInfo?.periode || "Semua"}</p>
                  <p className="text-[11px] text-black">Tgl Cetak: {currentDateStr}</p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between gap-4 bg-white p-2.5 border border-black page-break-inside-avoid">
                <div>
                  <h2 className="text-sm sm:text-base font-extrabold text-black uppercase tracking-wide">
                    LAPORAN RESUME PERMOHONAN TRANSAKSI (APPROVED 3)
                  </h2>
                  <p className="text-[11px] text-black font-medium">
                    Dokumentasi Rekapitulasi Otorisasi Berjenjang Tingkat Unit
                  </p>
                </div>
              ...
              <div className="p-3 bg-white border border-black text-[11px] grid grid-cols-2 sm:grid-cols-4 gap-2 text-black">
                <p><span>Unit UPT:</span> <strong>{reportFilterInfo?.unitUpt || "Semua"}</strong></p>
                <p><span>ULTG:</span> <strong>{reportFilterInfo?.unitUltg || "Semua"}</strong></p>
                <p><span>Jenis:</span> <strong>{reportFilterInfo?.type ? reportFilterInfo.type.toUpperCase() : "Semua"}</strong></p>
                <p><span>Status:</span> <strong className="text-emerald-800">APPROVED 3</strong></p>
              </div>
                ...
                <table className="w-full text-left text-[10px] border-collapse">
                  <thead>
                    <tr className="bg-slate-100 text-black font-extrabold border-b border-black">
                      ...
                      <th className="p-1.5 border-r border-black">NO. DOKUMEN</th>
                      <th className="p-1.5 border-r border-black">NAMA / NIP</th>
                      <th className="p-1.5 border-r border-black">JENIS</th>
                      <th className="p-1.5 border-r border-black">UNIT / ULTG</th>
                      <th className="p-1.5 border-r border-black">TGL PENGAJUAN</th>
                      <th className="p-1.5 border-r border-black text-right">RINCAN / NOMINAL</th>
                  ...
                  <tbody>
                    {reportSubmissions.length === 0 ? (
                      <tr>
                        <td colSpan={8} className="p-4 text-center text-black font-bold">
                          Tidak ada data permohonan Approved 3 untuk periode ini.
                        </td>
                      </tr>
                    ) : (
                      reportSubmissions.map((sub, idx) => (
                        <tr key={sub.id || idx} className="border-b border-black/50">
                          <td className="p-1.5 border-r border-black/50 text-center font-bold">{idx + 1}</td>
                          <td className="p-1.5 border-r border-black/50 font-mono font-bold">{getFormattedDocNo(sub)}</td>
                          <td className="p-1.5 border-r border-black/50 font-bold">{sub.employeeName} <br/><span className="font-normal font-mono text-[9px] text-black/80">{sub.employeeNip}</span></td>
                          <td className="p-1.5 border-r border-black/50 uppercase font-extrabold">{sub.type}</td>
                          <td className="p-1.5 border-r border-black/50">{sub.unitUpt} <br/><span className="text-[9px] text-black/80">{sub.unitUltg}</span></td>
                          <td className="p-1.5 border-r border-black/50">{formatDateIndonesian(sub.tanggalPengajuan)}</td>
                          <td className="p-1.5 border-r border-black/50 text-right font-mono font-bold">
                            {sub.type === "sppd" ? formatRupiah(sub.totalEstimasiBiaya || 0) : sub.type === "lembur" ? `${sub.durasiJam || 0} Jam` : `${sub.jumlahHari || 1} Hari`}
                          </td>
                          <td className="p-1.5 text-center font-bold text-emerald-800">APPROVED 3</td>
                        </tr>
                      ))
                    )}
                  </tbody>
              ...
              <div className="p-3 bg-white border border-black flex flex-wrap items-center justify-between gap-3 text-xs font-bold text-black">
                <div>Total SPPD Approved 3: <strong>{formatRupiah(reportTotalSppd)}</strong></div>
                <div>Total Durasi Lembur: <strong>{reportTotalLemburHours} Jam</strong></div>
                <div>Total Transaksi: <strong>{reportSubmissions.length} Berkas</strong></div>
              </div>
              ...
              <div className="space-y-2 pt-2 page-break-inside-avoid">
                <div className="border-b border-black pb-1">
                  <h4 className="text-xs font-black uppercase text-black tracking-wide">
                    LEMBAR PENGESAHAN LAPORAN RESUME
                  </h4>
                </div>
                <div className="grid grid-cols-3 gap-2 text-black">
                  {reportSignatories.map((sig, sIdx) => (
                    <div key={sIdx} className="border border-black p-2 flex flex-col justify-between h-36 bg-white">
                      <div>
                        <p className="text-[9px] font-bold border-b border-black pb-1 uppercase">{sig.role || `Penandatangan ${sIdx + 1}`}</p>
                        <p className="font-bold text-[10.5px] truncate mt-1">{sig.name || "-"}</p>
                        <p className="text-[9px] text-black/80 truncate">NIP. {sig.nip || "-"}</p>
                      </div>
                      <div className="my-0.5 h-10 flex items-center justify-center border border-dashed border-black/40">
                        {sig.signatureUrl ? (
                          <img src={sig.signatureUrl} alt="TTD" className="max-h-10 object-contain mix-blend-multiply" />
                        ) : (
                          <span className="text-[8.5px] font-bold text-black uppercase">[ TTD VALID DIGITAL ]</span>
                        )}
                      </div>
              ...
              <div className="pt-3 border-t border-black flex flex-col sm:flex-row items-center justify-between text-[10px] text-black gap-2 page-break-inside-avoid">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="font-bold text-black">DOKUMEN LAPORAN RESUME ELEKTRONIK</p>
                ...
                <div className="text-right font-mono text-[9.5px] flex flex-col items-end">
                  <img src={semarLogo} alt="SEMAR Logo" className="h-6 sm:h-7 w-auto object-contain flex-shrink-0 mb-0.5" referrerPolicy="no-referrer" />
                  <p className="text-black">Waktu Cetak: {new Date().toLocaleString("id-ID")}</p>
                </div>
              </div>
            </div>
          ) : (
            ...
            <div className="printable-a4-document max-w-[210mm] w-full mx-auto bg-white border border-black p-5 sm:p-6 space-y-4 rounded-none text-black">
              ...
              <div className="border-b-2 border-black pb-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 page-break-inside-avoid">
                ...
                <div className="text-left sm:text-right text-xs text-black border-l sm:border-l-0 pl-3 sm:pl-0 border-black">
                  <p className="font-extrabold text-black uppercase tracking-tight">FORMULIR PERSETUJUAN RESMI</p>
                  <p className="font-mono text-black font-bold">No: {finalDocNo}</p>
                  <p className="text-[11px] text-black">Tgl Pengajuan: {formatDateIndonesian(submission.tanggalPengajuan)}</p>
                </div>
              </div>
              ...
              {(submission.status?.toLowerCase() === "dibatalkan" || submission.status?.toLowerCase() === "rejected" || submission.rejectionReason) && (
                <div className="p-3 bg-white border border-black space-y-1 page-break-inside-avoid">
                  <p className="font-extrabold text-black uppercase tracking-tight text-xs flex items-center gap-1.5">
                    ...
                    {submission.status?.toLowerCase() === "dibatalkan" ? "KETERANGAN TRANSAKSI DIBATALKAN" : "KETERANGAN CATATAN PENOLAKAN"}
                  </p>
                  <p className="text-xs text-black font-semibold">
                    {submission.rejectionReason || (submission.status?.toLowerCase() === "dibatalkan" ? "Transaksi Dibatalkan oleh Pemohon" : "Pengajuan tidak disetujui.")}
                  </p>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs page-break-inside-avoid">
                <div className="bg-white p-3 border border-black space-y-1.5">
                  <p className="font-extrabold text-black border-b border-black pb-1 uppercase tracking-tight flex items-center gap-1.5">
                    <User className="w-3.5 h-3.5 text-black" /> BIODATA TENAGA KERJA
                  </p>
                  <div className="grid grid-cols-3 gap-1 text-[11px]">
                    <span className="text-black">Nama Lengkap</span>
                    <span className="col-span-2 font-bold text-black">: {submission.employeeName}</span>
                    <span className="text-black">NIP</span>
                    <span className="col-span-2 font-mono font-bold text-black">: {submission.employeeNip}</span>
                    <span className="text-black">Jabatan</span>
                    <span className="col-span-2 text-black">: {submission.employeeJabatan}</span>
                  </div>
                </div>

                <div className="bg-white p-3 border border-black space-y-1.5">
                  <p className="font-extrabold text-black border-b border-black pb-1 uppercase tracking-tight flex items-center gap-1.5">
                    <Building2 className="w-3.5 h-3.5 text-black" /> LOKASI UNIT KERJA
                  </p>
                  <div className="grid grid-cols-3 gap-1 text-[11px]">
                    <span className="text-black">Unit UPT</span>
                    <span className="col-span-2 font-semibold text-black">: {submission.unitUpt}</span>
                    <span className="text-black">ULTG</span>
                    <span className="col-span-2 text-black">: {submission.unitUltg}</span>
                    <span className="text-black">Gardu Induk</span>
                    <span className="col-span-2 font-bold text-black">: {submission.garduInduk}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white p-3 border border-black text-xs space-y-3 page-break-inside-avoid">
                <p className="font-extrabold text-black border-b border-black pb-1 uppercase tracking-tight flex items-center gap-1.5">
                  <Briefcase className="w-3.5 h-3.5 text-black" /> DETAIL DOKUMEN PENGAJUAN
                </p>
                ...
                {submission.type === "lembur" && (
                  <div className="space-y-3 text-black">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px]">
                      <p><span className="text-black">Tanggal Lembur:</span> <strong className="ml-1">{formatDateIndonesian(submission.tanggalLembur)}</strong></p>
                      <p><span className="text-black">Jam Operasional:</span> <strong className="ml-1">{submission.jamMulai} - {submission.jamSelesai} ({submission.durasiJam} Jam)</strong></p>
                      <p><span className="text-black">Kategori Lembur:</span> <strong className="ml-1">{submission.kategoriLembur}</strong></p>
                      {submission.jenisPekerjaan && submission.jenisPekerjaan !== "-" && (
                        <p><span className="text-black">Jenis Pekerjaan:</span> <strong className="ml-1">{submission.jenisPekerjaan}</strong></p>
                      )}
                      <p><span className="text-black font-semibold">Area Group:</span> <strong className="ml-1">{submission.areaGroup}</strong></p>
                      {submission.petugasPendampingNama && (
                        <p><span className="text-black">Ganti Piket An. :</span> <strong className="ml-1">{submission.petugasPendampingNip} - {submission.petugasPendampingNama}</strong></p>
                      )}
                      {submission.jumlahJamKoreksi !== undefined && submission.jumlahJamKoreksi !== null && submission.jumlahJamKoreksi !== "" && (
                        <p><span className="text-black">Jumlah Lembur Koreksi:</span> <strong className="ml-1 font-bold text-black">{submission.jumlahJamKoreksi} Jam</strong></p>
                      )}
                      {submission.catatanKoreksi && (
                        <p className="col-span-full"><span className="text-black font-semibold">Catatan Koreksi Checker:</span> {submission.catatanKoreksi}</p>
                      )}
                      <p className="col-span-full"><span className="text-black font-semibold">Uraian Pekerjaan:</span> {submission.kegiatanDetail}</p>
                    </div>
                  </div>
                )}
                ...
                {submission.type === "cuti" && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px] text-black">
                    <p><span className="text-black">Jenis Cuti:</span> <strong className="ml-1 text-black">{submission.cutiType}</strong></p>
                    <p><span className="text-black">Periode Cuti:</span> <strong className="ml-1">{formatDateIndonesian(submission.tanggalMulai)} s/d {formatDateIndonesian(submission.tanggalSelesai)}</strong></p>
                    <p><span className="text-black">Jumlah Durasi:</span> <strong className="ml-1">{submission.jumlahHari} Hari Kerja</strong></p>
                    <p><span className="text-black">Sisa Cuti Pasca Pengajuan:</span> <strong className="ml-1 text-black">{submission.sisaCutiSesudahnya} Hari</strong> (Sebelumnya: {submission.sisaCutiSebelumnya || "-"} Hari)</p>
                    <p className="col-span-full"><span className="text-black">Alamat Selama Cuti &amp; Kontak Darurat:</span> {submission.alamatSelamaCuti} (Telp: {submission.nomorTeleponDarurat})</p>
                  </div>
                )}
                ...
                {submission.type === "ijin" && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px] text-black">
                    <p><span className="text-black">Alasan / Jenis Ijin:</span> <strong className="ml-1 text-black">{submission.ijinReasonType || submission.alasanIjin || "Ijin Resmi"}</strong></p>
                    <p><span className="text-black">Jumlah Hari (Ijin):</span> <strong className="ml-1 text-sky-900 font-bold">{submission.jumlahHari} Hari</strong></p>
                    <p><span className="text-black">Jumlah Hari (di Setujui):</span> <strong className="ml-1 text-emerald-800 font-bold">{submission.jumlahHariDisetujui ? `${submission.jumlahHariDisetujui} Hari` : `${submission.jumlahHari} Hari (Proses)`}</strong></p>
                    <p className="col-span-full"><span className="text-black">Periode Ijin:</span> <strong className="ml-1">{formatDateIndonesian(submission.tanggalMulai)} s/d {formatDateIndonesian(submission.tanggalSelesai)}</strong></p>
                    <p className="col-span-full"><span className="text-black">Keterangan / Keperluan:</span> {submission.keterangan || "-"}</p>
                  </div>
                )}
                ...
                {submission.type === "sakit" && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px] text-black">
                    <p><span className="text-black">Instansi / Klinik / RS:</span> <strong className="ml-1 text-black">{submission.instansiKlinik || submission.namaDokterFaskes || "-"}</strong></p>
                    <p><span className="text-black">Dokter Penanggung Jawab:</span> <strong className="ml-1 text-black">{submission.namaDokter || "-"}</strong></p>
                    <p><span className="text-black">Periode Istirahat Sakit:</span> <strong className="ml-1">{formatDateIndonesian(submission.tanggalMulai)} s/d {formatDateIndonesian(submission.tanggalSelesai)} ({submission.jumlahHari} Hari)</strong></p>
                    <p><span className="text-black">Diagnosa Singkat:</span> <strong className="ml-1 text-black">{submission.diagnosaSingkat || "-"}</strong></p>
                  </div>
                )}
                ...
                {submission.type === "sppd" && (
                  <div className="space-y-3 text-black">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px]">
                      <p><span className="text-black">No. Surat Tugas PLN:</span> <strong className="ml-1 font-mono text-black">{submission.nomorSuratTugas}</strong></p>
                      <p><span className="text-black">Rute Perjalanan Dinas:</span> <strong className="ml-1">{submission.kotaAsal} &rarr; {submission.kotaTujuan} ({submission.durasiHari} Hari)</strong></p>
                      <p><span className="text-black">Tanggal Berangkat &amp; Kembali:</span> <strong className="ml-1">{formatDateIndonesian(submission.tanggalBerangkat)} s/d {formatDateIndonesian(submission.tanggalKembali)}</strong></p>
                      <p><span className="text-black">Beban Anggaran Unit:</span> <strong className="ml-1">{submission.bebanAnggaranUnit || "-"}</strong></p>
                      <p className="col-span-full"><span className="text-black">Maksud Perjalanan:</span> {submission.maksudPerjalanan}</p>
                    </div>
                      ...
                      <table className="w-full text-left text-[11px] border-collapse border border-black">
                        <thead>
                          <tr className="bg-white text-black font-extrabold border-b border-black">
                            <th className="p-1.5 border-r border-black">Rincian Komponen Biaya</th>
                        ...
                        <tbody>
                          {submission.expenses?.map((exp) => (
                            <tr key={exp.id} className="border-b border-black/40">
                              <td className="p-1.5 border-r border-black/40">{exp.deskripsi}</td>
                              <td className="p-1.5 border-r border-black/40">{exp.kategori}</td>
                              <td className="p-1.5 text-right font-mono font-bold">
                                {showSppdNominal ? formatRupiah(exp.nominal || 0) : "-"}
                              </td>
                            </tr>
                          ))}
                          <tr className="font-extrabold bg-white border-t border-black">
                            <td colSpan={2} className="p-1.5 text-right border-r border-black">TOTAL SPPD:</td>
                            <td className="p-1.5 text-right font-mono text-xs text-black">
                              {showSppdNominal ? formatRupiah(calculatedTotalSppd) : "-"}
                            </td>
                          </tr>
                        </tbody>
                      </table>
              ...
              <div className="space-y-3 pt-1 page-break-inside-avoid">
                ...
                <div className="flex justify-center">
                  <div className="w-full sm:w-1/3 max-w-xs">
                    ...
                    {renderSignatureBox("Maker / Pemohon", null, true)}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 pt-1">
                  <div className="bg-white p-2 border border-black space-y-1.5">
                    ...
                    <div className={`grid grid-cols-1 ${hasVerificationAssigned ? 'sm:grid-cols-3' : 'sm:grid-cols-2'} gap-2`}>
                      {renderSignatureBox(checkerStep)}
                      {hasVerificationAssigned && renderSignatureBox(verificationStep)}
              ...
              <div className="pt-3 border-t border-black flex flex-col sm:flex-row items-center justify-between text-[10px] text-black gap-2 page-break-inside-avoid">
                ...
                <div className="text-right font-mono text-[9.5px] flex flex-col items-end">
                  ...
                  <p className="text-black font-bold">
                    <span className="print:hidden">Halaman 1 dari {hasAttachment ? 2 : 1}</span>
        ...
        <div className="bg-slate-900 px-4 sm:px-6 py-3 border-t border-slate-800 flex items-center justify-between gap-3 text-white text-xs shrink-0 modal-controls">
          <div className="text-slate-400 font-medium hidden sm:block">
            {isReport ? (
              <span>Total Submissions Approved 3: <strong className="text-sky-400">{reportSubmissions.length} Transaksi</strong></span>
            ) : (
