  ...
  return (
    <div className="report-container font-sans text-black bg-white p-6">
      ...
      <header className="print-header-fixed mb-4">
        <div className="flex justify-between items-center border-b-2 border-black pb-2">
          ...
          <div className="text-right">
            ...
            <span className="text-xs font-mono font-semibold">{docNoStr}</span>
          </div>
        </div>
      </header>
      ...
      <main className="main-content my-4">
        {filterInfo && (filterInfo.month || filterInfo.type || filterInfo.unit) && (
          <div className="mb-3 text-xs border-b border-black pb-2 flex flex-wrap gap-4">
            {filterInfo.month && <div><span className="font-bold">Periode:</span> {filterInfo.month} {filterInfo.year || ''}</div>}
            {filterInfo.type && <div><span className="font-bold">Jenis:</span> {filterInfo.type.toUpperCase()}</div>}
            {filterInfo.unit && <div><span className="font-bold">Unit:</span> {filterInfo.unit}</div>}
          </div>
        )}
          ...
          <table className="w-full text-left text-xs border-collapse border border-black">
            <thead>
              <tr className="bg-slate-100 uppercase text-[10px] font-bold text-black">
                ...
                <th className="border border-black p-2">Tgl Pengajuan</th>
                ...
                <th className="border border-black p-2 text-right">Estimasi Biaya</th>
              </tr>
            </thead>
            <tbody>
              {data && data.length > 0 ? (
                data.map((item, idx) => {
                  let nominal = "-";
                  if (item.type === "lembur") nominal = formatRupiah(item.estimasiBiayaRupiah || 0);
                  if (item.type === "sppd") nominal = formatRupiah(item.totalEstimasiBiaya || 0);

                  return (
                    <tr key={item.id || idx} className="hover:bg-slate-50 text-[11px]">
                      <td className="border border-black p-2 text-center">{idx + 1}</td>
                      <td className="border border-black p-2 font-mono">{getFormattedDocNo(item)}</td>
                      <td className="border border-black p-2">{item.tanggalPengajuan ? formatDateIndonesian(item.tanggalPengajuan) : "-"}</td>
                      <td className="border border-black p-2">
                        <div className="font-bold">{item.employeeName || item.namaPegawai || "-"}</div>
                        <div className="text-[10px] text-slate-600 font-mono">{item.employeeNip || item.nip || ""}</div>
                      </td>
                      <td className="border border-black p-2 uppercase font-semibold">{item.type || item.jenisPermohonan || "-"}</td>
                      <td className="border border-black p-2">{item.unitKerja || item.unitUltg || item.unitUpt || item.unit || "-"}</td>
                      <td className="border border-black p-2">{item.keterangan || item.maksudSppd || item.alasan || "-"}</td>
                      <td className="border border-black p-2 text-right font-bold">{nominal}</td>
                    </tr>
                  );
                })
              ) : (
        ...
        {(() => {
          ...
          const defaultSignatories = [
            {
              ...
              jabatan: `AMN PLN ${filterInfo.unit || "UPT Semarang"}`
            },
            {
              ...
              jabatan: `TL ES ${filterInfo.unit || "UPT Semarang"}`
            },
            {
              ...
              jabatan: `AMN ES ${filterInfo.unit || "UPT Semarang"}`
            }
          ];
          ...
          const validSignatories = rawSigs.filter((sig, idx) => {
          ...
          if (!validSignatories || validSignatories.length === 0) return null;
          const colsClass = validSignatories.length === 2 ? "grid-cols-2" : validSignatories.length === 1 ? "grid-cols-1" : "grid-cols-3";

          return (
            <div className={`mt-8 grid ${colsClass} gap-4 text-center text-xs page-break-inside-avoid`}>
              {validSignatories.map((sig, idx) => (
                <div key={idx} className="flex flex-col items-center justify-between h-28 border border-black p-2">
                  <span className="font-bold text-[10px] uppercase">{sig.title || sig.positionLabel || `Penandatangan ${idx + 1}`}</span>
                  <div className="my-1 border-b border-black w-24"></div>
                  <div>
                    <p className="font-bold underline">{sig.name || "(.........................)"}</p>
                    <p className="text-[9px] font-mono">{sig.nip ? `NIP. ${sig.nip}` : sig.jabatan || ""}</p>
                  </div>
                </div>
              ))}
            </div>
